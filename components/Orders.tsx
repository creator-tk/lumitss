    ...
    <div>
      ...
      <ul className='overflow-y-scroll'>
        {orderedItems.map((item, index) => {
          ...
          return (
            <div key={item.$id}>
              <li key={item.$id} className='grid grid-cols-10 gap-4 items-center mb-3'>
                <Image
                  src={item.image}
                  alt="product"
                  width={100}
                ...
                <p className='col-span-1 text-green-400 font-semibold'>{order?.orderStatus}</p>
              </li>
              <div className='flex justify-between w-[100%] lg:flex-row flex-col'>
                <p>Ordered On: {formateDateTime(order?.orderDate)}</p>
                <p>Delivered By: <span className='text-green-400 font-bold'>{formateDateTime(order?.orderDate, 7)}</span></p>
              </div>
              <hr className='my-4' />
            </div>
          );
